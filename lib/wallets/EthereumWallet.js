...
class EthereumWallet extends Wallet {
	static parseArgs(args) {
		args.gasPrice = ethers.utils.bigNumberify(ethUnit.toWei(args.gasPrice, 'gwei').toString());
		args.gasLimit = ethers.utils.bigNumberify(ethUnit.toWei(args.gasLimit, 'mwei').toString());
	...
	static get etherscanUrl() {
		return `https://${
			process.env.ETHEREUM_NETWORK === 'mainnnet' ? '' : process.env.ETHEREUM_NETWORK + '.'
		}etherscan.io/`;
	}
	...
	static async withdraw({ amount, address, gasPrice }) {
		...
		console.log(`Transaction Submitted ${this.etherscanUrl}tx/${response.hash}`);
	...
	static async deploy(args) {
		...
		console.log('Contract Transaction Successful', `${this.etherscanUrl}tx/${response.hash}`);
	}
}
