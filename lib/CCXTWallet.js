...
class CCXTWallet extends Exchange {
	...
	static async withdraw({ amount, address, twoFactor, fee }) {
		return await this.exchange.withdraw(this.symbol, amount, address, undefined, {
			otpToken: twoFactor,
			fee
		});
	}
	...
	static async balance() {
		...
		return { [this.symbol]: response[this.symbol] };
	}
	...
	static async walletBalance() {
		...
		return BigNumber(response[this.tradingPair[0]].free);
	}

	static async totalBalance() {
		const response = await this.exchange.fetchBalance();

		return BigNumber((response[this.tradingPair[1]] || { total: 0 }).total.toString());
	}

	static async buyBalance() {
		const response = await this.exchange.fetchBalance();

		return BigNumber((response[this.tradingPair[1]] || { free: 0 }).free.toString());
	}

	static async sellBalance() {
		const response = await this.exchange.fetchBalance();

		return BigNumber((response[this.tradingPair[0]] || { free: 0 }).free.toString());
	}
