...
class Exchange extends Wallet {
	...
	static async buy({ amount, price }, params = {}) {
		...
		console.log(`Buy ${payload.amount} ${this.tradingPair[0]} at ${payload.price} ${this.tradingPair[1]} for a total of ${new BigNumber(payload.amount).times(payload.price).toFixed(2)} ${this.tradingPair[1]}?`);
	...
	static async sell({ amount, price }, params = {}) {
		...
		console.log(`Sell ${payload.amount} ${this.tradingPair[0]} at ${payload.price} ${this.tradingPair[1]} for a total of ${new BigNumber(payload.amount).times(payload.price).toFixed(2)} ${this.tradingPair[1]}?`);
		console.log(`Type 'yes' to continue`)
		prompt.start();
